...
**rq2.csv** (methodology: "v4" reconstructed to an external 9-column spec, superseding
v3's column set - see rq2 section below; numbers as of the 2026-08-25 backfill,
which read back the same 2026-08-24 rerun's Step 1/3 artifacts, no re-execution)

| project | bug_id | full_execution_size | passing_TC_size | failing_TC_size | union_passing_slice | union_failing_slice | union_all_slice | all_reduction_ratio |
|---|---|---|---|---|---|---|---|---|
| Csv | 3b | 443 | 439 | 215 | 204 | 148 | 211 | 0.523702 |
| Csv | 13b | 609 | 609 | 112 | 1 | 59 | 59 | 0.90312 |
| JacksonXml | 1b | 1198 | 1194 | 395 | 120 | 107 | 124 | 0.896494 |
| JacksonXml | 6b | 1334 | 1330 | 300 | 65 | 4 | 65 | 0.951274 |
...
### v4 — reconstructed to an externally-supplied 9-column spec (CURRENT/FINAL, code changed 2026-08-25)

Superseded v3 wholesale on a full external task spec (`project, bug_id,
full_execution_size, passing_TC_size, failing_TC_size, union_passing_slice,
union_failing_slice, union_all_slice, all_reduction_ratio` - lowercase,
exactly 9 columns, `Pass_TC_Slices`/`Fail_TC_Slices` explicitly *not*
wanted). Two changes from v3, not just a rename:

```
full_execution_size, union_passing_slice, union_failing_slice,
union_all_slice, all_reduction_ratio
    = v3's Full_Execution_Size / Union_Passing_Slices / Union_Failing_Slices
      / Union_All_Slices / Reduction_Ratio, same numbers, renamed only
      (v3's TC-grouped mapping was already what this spec's section 6/7
      describe: slices of assertions "belonging to" a passing/failing test
      case, i.e. grouped by the SOURCE TEST CASE's outcome).

passing_TC_size / failing_TC_size
    = GENUINELY NEW, not previously computed anywhere: unique statements in
      the RAW (unsliced) trace-matrix coverage (Step 3a), unioned over all
      PASSING test cases and separately all FAILING test cases. Read
      straight off step3_matrices.build_trace_matrix's per-test-case 1/0
      columns, now exposed from step3_matrices.run() as ochiai_result
      ["trace_matrix_rows"] (previously only Full_Execution_Size - the
      cross-test union size - left that function; no per-test breakdown
      escaped it before). Confirmed passing_TC_size ∪ failing_TC_size ==
      full_execution_size for all 4 targets (every test is PASS xor FAIL,
      never anything else - see step1_tests.py's `_run_single_test`).

Pass_TC_Slices/Fail_TC_Slices (v3's assertion-outcome-grouped pair) are
DROPPED, not renamed - this spec has no column for that grouping and
explicitly says not to add extra columns beyond the 9 listed.
```

**rq2.csv was not regenerated by rerunning `run_pipeline.py`** (that reruns
`defects4j test`/`coverage`/Slicer4J - minutes per target, see rq3.csv's
timings below - and none of Step 1-3's raw execution/slicing data actually
needed to change for this, only which numbers rq_writers.py reads back out
of it). Instead a one-off script read the same 4 targets' already-computed
`resQ_outputs/work/<target>/step1_tests/test_results.csv` and
`step3_matrices/{trace_statement_mapping,trace_observation_matrix,
slice_statement_mapping,slice_observation_matrix,virtual_test_status}.csv`
back into the same shapes `rq_writers._write_rq2` expects, and called that
real function directly (not a second, divergent reimplementation of the
formulas) - consistent with rq_writers.py's own "nothing here re-runs any
tool" contract. `resQ_outputs/rq2.csv` was deleted and rewritten fresh
first, since `upsert_csv_row`'s key-based replace can't match old
PascalCase `Project`/`BugID` rows against new lowercase `project`/`bug_id`
keys - upserting onto the stale-schema file directly leaves 4 blank
leftover rows (verified by reproducing it once, then fixed by starting
from an empty file).

**Current values (2026-08-25 backfill, v4 schema):**
